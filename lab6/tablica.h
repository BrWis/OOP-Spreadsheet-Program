...
/**
  ...
  * @param typ  zmienna przechowująca informację o typie kolumn tej tablicy
  */
class Tablica
{
	private:
	...
	public:
		/**
		  ...
		  * @param liczba_wierszy  liczba wierszy utworzonej tablicy
  		  * @param typ  zmienna przechowująca informację o typie kolumn utworzonej tablicy
		  */
		Tablica(int liczba_kolumn = 0, int liczba_wierszy = 0, string typ = "")
		{
			if(typ.length() != liczba_kolumn)
			{
				throw std::invalid_argument("Podano niewłaściwą liczbę kolumn");
			}
			for(int i = 0; i < typ.length(); i++)
			{
				...
				{ 
					...
					throw std::invalid_argument("Podano niewłaściwe typy, podaj tylko wartości 'i' lub 's' ");
				}
			}
		 ...
		 * @param nowy_typ  typy kolumn nowej tablicy
 		 */
		void modyfikujRozmiar(int nowy_kolumny, int nowy_wiersze, string nowy_typ);
